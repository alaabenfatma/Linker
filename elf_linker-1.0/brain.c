...
//presque pareil que load_data 
void load_data2(FILE *file, Elf32_Ehdr* header, Elf32_Shdr* section)
{
   if (file)
   {
      if (!fread(header, 1, sizeof(header), file))
      ...
      fseek(file, (header->e_shoff) + (header->e_shstrndx) * sizeof(Elf32_Shdr), SEEK_SET);
      if (!fread(section, 1, sizeof(Elf32_Shdr), file))
      {
         printf("Error section");
         exit(1);
      }
...
//presque pareil que get_tab_symb
int get_tab_symb2(FILE *file, Elf32_Shdr section,Elf32_Ehdr header)
{
...
int has_multiple_declarations(FILE* f1, FILE* f2){
    ...
    load_data2(f1,&header,&section1);
    int indice_tab_symb1 = get_tab_symb2(f1,section1,header1);
   ...
   fseek(f1, section1.sh_offset, SEEK_SET);
    ...
    load_data2(f2,&header,&section2);
    int indice_tab_symb2 = get_tab_symb2(f2,section2,header2);
